...
export default function NewTaskModal({ isOpen, onClose, currentUser, onTaskCreated }: any) {
  // 1. Adicionei 'source' no estado inicial como "Rotina"
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 2. Enviando a origem para o backend
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
      <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in duration-200">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-4">
          ...
          {/* LINHA 1: Responsável e Origem */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Responsável</label>
              <select required className="w-full p-4 bg-slate-50 border rounded-2xl font-bold text-sm outline-none mt-1 focus:border-orange-500"
            ...
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Origem</label>
              <select className="w-full p-4 bg-slate-50 border rounded-2xl font-bold text-sm outline-none mt-1 focus:border-orange-500"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required type="date" className="w-full p-4 bg-slate-50 border rounded-2xl font-bold text-sm outline-none mt-1 focus:border-orange-500" 
