...
import { Clock, CheckCircle2, AlertCircle, PlayCircle, Plus, Search, FileSpreadsheet } from 'lucide-react';
import ImportModal from '../components/ImportModal'; // <--- 1. Importei aqui
...
export default function TaskList({ onOpenTask, viewMode, currentUser }: any) {
  ...
  const [isImportOpen, setIsImportOpen] = useState(false); // <--- 2. Estado do Modal
  const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3000';

  const operations = ["Todos", "Geral","Financeiro", "Tim", "Samsung", "Automação"];
  ...
  const filteredTasks = tasks.filter(task => {
     ...
     const responsibleOp = task.operation || "Outros";
     const creatorOp = task.creatorOperation || "Outros";

     const matchesSearch = title.includes(search) || user.includes(search);
     const matchesOp = filterOp === 'Todos' || responsibleOp === filterOp || creatorOp === filterOp;
    ...
    <div className="h-full overflow-y-auto p-8 relative">
      ...
      <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-4 items-center">
              ...
              {/* 3. BOTÃO DE IMPORTAR (NOVO) */}
              <button onClick={() => setIsImportOpen(true)} className="bg-green-600 text-white px-4 py-3 rounded-2xl font-black text-xs uppercase flex gap-2 hover:bg-green-700 shadow-lg transition-all active:scale-95">
          ...
          <div key={task.id} onClick={() => onOpenTask(task)} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm hover:shadow-md transition-all cursor-pointer flex items-center gap-6 group">
            ...
            <div className="flex-1">
                ...
                <div className="flex gap-4 mt-1 items-center">
                    ...
                    {/* Se tiver criador diferente do usuário atual, pode mostrar aqui se quiser */}
                    {task.creatorOperation && task.creatorOperation !== "Outros" && (
                         <span className="text-[9px] font-bold text-slate-400 uppercase tracking-tighter ml-2 border-l pl-2">
                             Origem: {task.creatorOperation}
                         </span>
                    )}
                </div>
            </div>
      ...
      {/* 4. COMPONENTE MODAL DE IMPORTAÇÃO (NOVO) */}
      <ImportModal 
         isOpen={isImportOpen} 
